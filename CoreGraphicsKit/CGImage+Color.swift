...
extension CGImage {
    ...
    public func getDominantColor(quality: Int = defaultQuality, ignoreWhite: Bool = defaultIgnoreWhite) -> CGColor? {
        guard let palette = self.getPalette(colorCount: 5, quality: quality, ignoreWhite: ignoreWhite) else {
            return nil
        }
    ...
    ///   - ignoreWhite: if true, white pixels are ignored
    /// - Returns: the palette
    public func getPalette(colorCount: Int, quality: Int = defaultQuality, ignoreWhite: Bool = defaultIgnoreWhite) -> [CGColor]? {
        guard let colorMap = self.getColorMap(colorCount: colorCount, quality: quality, ignoreWhite: ignoreWhite) else {
            return nil
        }
    ...
    private func getColorMap(colorCount: Int, quality: Int = defaultQuality, ignoreWhite: Bool = defaultIgnoreWhite) -> MMCQ.ColorMap? {
        guard let pixels = self.makeBytes() else {
            return nil
        }
    ...
    private func makeBytes() -> [UInt8]? {
        if self.isCompatibleImage() {
            return self.makeBytesFromCompatibleImage()
        } else {
            return self.makeBytesFromIncompatibleImage()
        }
    }

    private func isCompatibleImage() -> Bool {
        ...
        if self.bytesPerRow != self.width * 4 {
            return false
        }
        return true
    }

    private func makeBytesFromCompatibleImage() -> [UInt8]? {
    ...
    private func makeBytesFromIncompatibleImage() -> [UInt8]? {
...
private class MMCQ {
    ...
    class ColorMap {
        ...
        func makePalette() -> [CGColor] {
            return vboxes.map { $0.getAverage().makeCGColor() }
        }
